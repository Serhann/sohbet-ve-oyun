	...
	async run(msg, args) {
        ...
        if (args.channel) {
			...
			if (vt === args.channel.id) {
				...
				msg.channel.send(`${client.config.customEmojis.basarisiz} Log kanalı zaten **${args.channel.name}** olarak ayarlı.`);
			} else {
				this.client.provider.set(msg.guild.id, 'logsChannel', args.channel.id);
				this.client.provider.set(msg.guild.id, 'logsEnable', true);
				return msg.channel.send(`${client.config.customEmojis.basarili} Log olarak ayarlanan kanal: **${args.channel.name}**`);
			}
        }
    }
};